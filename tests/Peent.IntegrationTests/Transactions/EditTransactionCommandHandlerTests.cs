...
{
    public class EditTransactionCommandHandlerTests : IntegrationTestBase
    {
        [Fact]
        public async Task should_update_transaction()
        {
            ...
            var createCommand = F.Build<CreateTransactionCommand>()
            ...
            var transactionId = await SendAsync(createCommand);

            var command = F.Build<EditTransactionCommand>()
                .With(x => x.Id, transactionId)
                .With(x => x.CategoryId, categoryId2)
                .With(x => x.CurrencyId, _currencyId)
                .With(x => x.ForeignCurrencyId, (int?)null)
                .With(x => x.FromAccountId, accountId3)
                .With(x => x.ToAccountId, accountId4)
                .Create();
        ...
        [Fact]
        public async Task should_update_transaction_by_another_user_in_the_same_workspace()
        {
            var user = await CreateUserAsync();
            ...
            var transaction = await FindAsync<Transaction>(transactionId);
            transaction.ModificationInfo.LastModifiedById.Should().Be(user2.Id);

            var entries = await GetAsync<TransactionEntry>(x => x.TransactionId == transactionId);
            foreach (var entry in entries)
            {
                entry.ModificationInfo.LastModifiedById.Should().Be(user2.Id);
            }
        }

        [Fact]
        public async Task when_transaction_is_edited__two_entries_have_amount_sum_equal_zero()
        {
            var user = await CreateUserAsync();
            SetCurrentUser(user, await CreateWorkspaceAsync(user));
            var categoryId = await SendAsync(F.Create<CreateCategoryCommand>());
        ...
        [Fact]
        public async Task when_transaction_is_edited__lastModifiedBy_is_set_to_current_user()
        {
            var user = await CreateUserAsync();
            SetCurrentUser(user, await CreateWorkspaceAsync(user));
            var categoryId = await SendAsync(F.Create<CreateCategoryCommand>());
            ...
            var transaction = await FindAsync<Transaction>(transactionId);
            transaction.ModificationInfo.LastModifiedById.Should().Be(user.Id);
            var entries = await GetAsync<TransactionEntry>(x => x.TransactionId == transactionId);
            foreach (var entry in entries)
            {
                entry.ModificationInfo.LastModifiedById.Should().Be(user.Id);
            }
        }
        ...
        [Fact]
        public async Task when_transaction_is_modified__workspace_is_set_to_current_user_workspace()
        {
            var user = await CreateUserAsync();
            var workspace = await CreateWorkspaceAsync(user);
            SetCurrentUser(user, workspace);
        ...
        [Fact]
        public async Task when_transaction_is_modified__entry_with_negative_amount_should_have_account_set_to_from_account()
        {
            var user = await CreateUserAsync();
            SetCurrentUser(user, await CreateWorkspaceAsync(user));
            var categoryId = await SendAsync(F.Create<CreateCategoryCommand>());
        ...
        [Fact]
        public async Task when_transaction_is_modified__entry_with_positive_amount_should_have_account_set_to_to_account()
        {
            var user = await CreateUserAsync();
            SetCurrentUser(user, await CreateWorkspaceAsync(user));
            var categoryId = await SendAsync(F.Create<CreateCategoryCommand>());
